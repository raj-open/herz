...
class PolyBase(Generic[T]):
    ...
    _roots: list[complex] = field(init=False, repr=True)
...
class Poly(PolyBase[T]):
    ...
    def __add__(self, q: Any) -> Poly[T]:
        ...
        if isinstance(q, Poly):
            ...
            if q.lead == 0:
                result.roots = self.roots
        raise ValueError(f'No addition method for Poly + {type(q)}!')
    ...
    def __mul__(self, q: Any):
        ...
        if isinstance(q, Poly):
            ...
            return result
        raise ValueError(f'No multiplication method for Poly x {type(q)}!')
