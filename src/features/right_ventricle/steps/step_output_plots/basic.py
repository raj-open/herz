...
def compute_fitted_curves_poly(
    ...
    data = np.row_stack(
        [np.concatenate([cv_value * p.values(time), [None]] * len(cycles)) for p in polys]
    )
    ...
    for k, p in enumerate(polys):
        special_ = {
            ...
            if key == "align"
            or (not point.ignore and (point.derivatives is None or k in point.derivatives))
        }
...
def compute_fitted_curves_trig(
    ...
    for per, a, b in resolution:
        ...
        values_ = fit.vshift + fit.drift * time_ + fit.vscale * np.cos(omega * (time_ - fit.hshift))  # fmt: skip
