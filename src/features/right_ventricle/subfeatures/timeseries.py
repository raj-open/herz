...
def subfeature_time_series_steps(
    ...
    interpols_trig: dict[
        str, tuple[FittedInfoTrig | None, list[tuple[float, float]], list[tuple[float, float]]]
    ],
...
def subfeature_time_series_steps_single(
    ...
    data, fitsinfos_poly = step_fit_poly(
        data, quantity=quantity, conds=conds, n_der=2, deg=None, mode=mode
    )
    poly, _ = fitsinfos_poly[-1]
    subprog.next()

    subprog = prog.subtask(
        f"""RECOGNISE CRITICAL POINTS OF {quantity} VIA POLY-FITTING""", steps=1
    )
    ...
    if cfg_poly is not None:
        ...
        subprog = prog.subtask(
            f"""RE-RECOGNISE CRITICAL POINTS OF {quantity} VIA POLY-FITTING""", steps=2
        )
        special, points_data = step_recognise_points(data, fitinfos=fitsinfos_poly, cfg=cfg_points, key_align=key_align)  # fmt: skip
        subprog.next()
    ...
    if cfg_trig is not None:
        subprog = prog.subtask(
            f"""INTERPOLATE TRIG-CURVE + COMPUTE ISO-MAX FOR {quantity}""", steps=2
        )
    ...
    if interp_trig is not None:
        ...
        hull_trig = get_realignment_intervals(
            hull_trig, info=info, special=special, collapse=False
        )
        intervals_trig = get_realignment_intervals(
            intervals_trig, info=info, special=special, collapse=False
        )
    subprog.next()
